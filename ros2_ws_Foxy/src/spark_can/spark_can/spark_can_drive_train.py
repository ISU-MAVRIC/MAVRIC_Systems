...
def talker(args=None):
    ...
    Pos_pub = node.create_publisher(ArmData, "Arm/JointPosition", 10)
    ...
    while rclpy.ok():
        ...
        strpub()
        spark_shoulderRot.percent_output(
            c_ShoulderRot * ShoulderRot * c_ShoulderRotDir / 100
        )
        spark_shoulderPitch.percent_output(
            c_ShoulderPitch * ShoulderPitch * c_ShoulderPitchDir / 100
        )
        spark_elbowPitch.percent_output(
            c_ElbowPitch * ElbowPitch * c_ElbowPitchDir / 100
        )
        spark_wristPitch.percent_output(
            c_WristPitch * WristPitch * c_WristPitchDir / 100
        )
        if snapping:
        ...
        spark_DrillActuator.percent_output(
            c_DrillActuator * DrillActuator * c_DrillActuator / 100
        )
